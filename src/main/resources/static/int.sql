...
-- =========================
-- 旧数据迁移：books.category -> categories + book_categories
-- 必须放在 books 插入之后
-- 可重复执行：通过 UNIQUE 和 IGNORE 避免重复
-- =========================
INSERT IGNORE INTO `categories` (`name`, `created_at`, `updated_at`)
SELECT DISTINCT TRIM(`category`) AS `name`, NOW(), NOW()
FROM `books`
...
SELECT b.id, c.id
FROM `books` b
JOIN `categories` c ON c.name = TRIM(b.category)
